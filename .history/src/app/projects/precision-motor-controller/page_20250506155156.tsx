...
// Define consistent types across all project pages
...
interface DatabaseProjectItem {
  title: string;
  description: string; // Mapped to summary
  technologies: string[];
  imageSrc: string;
  challenge?: string;
  solution?: string;
...
const DEFAULT_CHALLENGE = "Detailed challenge information will be available soon.";
const DEFAULT_SOLUTION = "A comprehensive solution was implemented.";
const DEFAULT_RESULTS = "The project achieved significant results.";
...
const DEFAULT_TESTIMONIAL = {
  quote: "We are very pleased with the outcome.",
  name: "Valued Client",
  position: "Management",
...
export default function PrecisionMotorControllerPage() {
  ...
  const [content, setContent] = useState<ProjectSpecificContent | null>(null); // Use correct type
  ...
  const PROJECT_PAGE_IDENTIFIER = "Precision Motor Controller"; // Define identifier

  useEffect(() => {
    const authStatus = sessionStorage.getItem('isAuthenticated');
    setIsAuthenticated(authStatus === 'true');
    fetchProjectContent(); // Fetch using Supabase
  }, []);

  // Standardized fetch function using Supabase
  const fetchProjectContent = async () => {
    ...
    try {
      ...
      if (!fullDataWrapper || !fullDataWrapper.data) throw new Error("No content data found");
      ...
      if (projectDataFromDb) {
        ...
        console.error(`Project data for "${PROJECT_PAGE_IDENTIFIER}" not found.`);
        setContent({ 
            title: PROJECT_PAGE_IDENTIFIER,
            summary: "Details not loaded. Check database.",
  ...
  // Standardized handleContentChange
  const handleContentChange = (field: keyof ProjectSpecificContent, value: any) => {
    setContent(prev => {
      ...
      if (field === 'testimonial' && typeof value === 'object' && value !== null && prev.testimonial) {
         // Special handling for nested testimonial fields if needed, or just update the whole object
         // This simple update assumes the whole testimonial object is passed in value
        return { ...prev, testimonial: { ...prev.testimonial, ...value } };
      }
      // Add similar handling for additionalImages array if complex updates are needed
      return { ...prev, [field]: value };
    });
  };
  
  // Standardized saveContent function using Supabase
  const saveContent = async () => {
    ...
    try {
      ...
      if (projectIndex === undefined || projectIndex === -1) {
        throw new Error(`Could not find project "${PROJECT_PAGE_IDENTIFIER}" in database.`);
      }
  ...
  // Authentication handlers (handleAuthenticate, handleLogout) remain the same as standardized previously
  const handleAuthenticate = (e: React.FormEvent) => {
    e.preventDefault();
    if (password === 'George123') { // Use env var ideally
  ...
  // Render logic (isLoading, !content checks) remains the same
  if (isLoading) {
  ...
  // JSX Return Structure (should be adapted to keep existing layout but use standardized state/functions)
  return (
    <>
      {/* Auth Modal (keep existing structure) */} 
      {isAuthModalOpen && (
         <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-70 px-4"> {/* Increased z-index for modal */} 
      ...
      {/* Admin Toolbar (keep existing structure, ensure z-index is z-60) */} 
      {isAuthenticated && ( 
      ...
      {/* Padlock Icon (keep existing structure) */} 
      {!isAuthenticated && ( 
      ...
      {/* Main Content Area (keep existing structure, ensure top padding adjusts for admin bar) */} 
      <div className={`bg-b0ase-dark text-gray-300 min-h-screen ${isAuthenticated ? 'pt-16 sm:pt-12' : ''}`}> 
        <main className="container mx-auto px-4 py-12 md:py-20"> 
           {/* Back to Projects Link */} 
           <div className="mb-8"> 
             <Link href="/newsite#projects" className="border border-gray-700 text-white text-sm px-4 py-2 hover:bg-gray-800 transition inline-flex items-center rounded-md"> 
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12"> 
             {/* Left Column (Main Image, Sections) */} 
             ...
             {/* Right Column (Aside - Technologies, Testimonial, CTA) */} 
             <aside className="lg:col-span-1 space-y-8"> 
               ...
               <div className="bg-b0ase-card p-6 rounded-lg border border-b0ase-card-border shadow-md"> 
                 ...
                 <p className="text-sm text-gray-400 mb-5 leading-relaxed">Need a high-performance motor control solution? Contact us to discuss your needs.</p> 
