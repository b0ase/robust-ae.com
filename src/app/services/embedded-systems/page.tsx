import React from 'react';
import Link from 'next/link';

export default function EmbeddedSystemsPage() {
  return (
    <div className="container mx-auto px-4 py-16">
      <Link href="/newsite" className="text-blue-600 hover:underline mb-8 inline-block">&larr; Back to Services</Link>
      <h1 className="text-3xl md:text-4xl font-bold mb-6">Embedded Systems Development</h1>
      <p className="text-lg text-gray-700">
        From initial concept and microcontroller selection to firmware development and testing, we deliver robust and efficient embedded solutions tailored to your specific needs.
      </p>
      {/* Add more details about this service here */}
    </div>
  );
} 